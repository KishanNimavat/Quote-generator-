...
document.querySelector("#generates").addEventListener("click", () => {
  generates();
});

function generates() {
  // grab all the keys in the dictionary (authors) and store in an array
  const authors = Object.keys(quotess);
  // grab a random key (author) and store it in author
  const author = authors[Math.floor(Math.random() * authors.length)];
  // grab the value(quote) that belongs to that key
  const quote = quotess[author]
...
document.querySelector("#geenerate").addEventListener("click", () => {
  geenerate();
});

function geenerate() {
  // grab all the keys in the dictionary (authors) and store in an array
  const authors = Object.keys(quotesss);
  // grab a random key (author) and store it in author
  const author = authors[Math.floor(Math.random() * authors.length)];
  // grab the value(quote) that belongs to that key
  const quote = quotesss[author]

  document.querySelector("#quuote").textContent = quote;
  document.querySelector("#auuthor").textContent = author;
}
